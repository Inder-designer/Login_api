...
router.post('/save-user', async (req: Request | any, res: Response) => {
    const { username, email, firstName, lastName, password, phoneNumber, language } = req.body;
    const controller = new ClientController(req, res)
    const response = await controller.saveUser({ username, email, firstName, lastName, password, phoneNumber, language });
